...
class UserTest(BaseTest):
    ...
    def test_sign_up(self):
        ...
        signup_s = self.app.test_client().post('/api/v1/account/signup', json={
            'email': 'test@test.com',
            'username': 'tests',
            'password': '12345'
        })
    ...
    def test_user_sign_in(self):
        self.app.test_client().post('/api/v1/account/signup', json={
            'email': 'test@test.com',
            'username': 'tests',
            'password': '12345'
        })
        signin = self.app.test_client().post('/api/v1/account/signin', json={
            'email': 'test@test.com',
            'password': '12345'
        })
    ...
    def test_get_user(self):
        ...
        if access_token is not None:
            user = self.app.test_client().get('/api/v1/account/me', headers={
                "Authorization": "Bearer {}".format(access_token['token'])
            })
            self.assertEqual(user.status_code, 200)

    def test_mal_get_user(self):
        me = self.app.test_client().get('/api/v1/account/me', headers={
            "Authorization": "Bearer bad token"
        })
        self.assertEqual(me.status_code, 422)
        pass
    ...
    def test_mal_signup(self):
        ...
        signup = self.app.test_client().post('/api/v1/account/signup', json={
            'email': 'test@test.com',
            'username': "test",
            'password': "12345"
        })
    ...
    def test_mal_signin(self):
        ...
        # verify that we don't have token when email is wrong
        responses = signin.get_data(as_text=True)
        self.assertFalse("token" in responses)

        # test with wrong password
        signin_password = self.app.test_client().post('/api/v1/account/signin', json={
            'email': 'test@test.com',
            'password': '1234'
        })
        ...
        # verify that we don't have access token when password is wrong
